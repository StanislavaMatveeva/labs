...
namespace Presentation
{
    ...
    public partial class Presentation : Form
    {
        ...
        public Presentation(IFacade newFacade)
        {
            InitializeComponent();
            facade = newFacade;
        ...
        /*public Form1(IFacade facade) : this()
        {
            Facade = facade;
        ...
        {
            MessageBox.Show(message, "Presentation", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        private void CreateStartStudentsTable()
        {
            ...
            //dataGridViewStudents.Columns.Add("group id", "Group ID");
            foreach (var s in facade.Students)
            {
                dataGridViewStudents.Rows.Add(s.Id, s.Name, s.Age, s.Group.Name);
                //dataGridViewStudents.Rows.Add(s.Id, s.Name, s.Age, s.GroupId);
            }
        }
        ...
        private void CreateStartCuratorsTable()
        {
            ...
            //dataGridViewCurators.Columns.Add("groupId", "Group ID");
            foreach (var c in facade.Curators)
                dataGridViewCurators.Rows.Add(c.Id, c.Name, c.Email, c.Group.Name);
            //dataGridViewCurators.Rows.Add(c.Id, c.Name, c.Email, c.GroupId);
        }
        ...
        /// <summary>
        /// Calls method that creates table with new data from table "Students".
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonSaveStudentsTable_Click(object sender, EventArgs e)
        {
            //UpdateStudentsTable();
        }
        ...
        /// <summary>
        /// Calls method that creates table with new data from table "Curators".
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonSaveCuratorsTable_Click(object sender, EventArgs e)
        {
            //UpdateCuratorsTable();
        }
        ...
        private void AddStudentToTable(int rowIndex)
        {
            ...
            //if (!int.TryParse(dataGridViewStudents.Rows[rowIndex].Cells[studentGroupIdColumn].Value.ToString(), out int groupId))
            //{ TellAboutError("Wrong value of student's groupId"); return; }

            //try { facade = facade.AddStudent((Facade)facade, new Student(newIdByDefault, name, age, groupId)); }
            //catch (Exception ex) { TellAboutError(ex.Message); return; }
        }
        ...
        private void AddCuratorToTable(int rowIndex)
        {
            ...
            //if (!int.TryParse(dataGridViewCurators.Rows[rowIndex].Cells[curatorGroupIdColumn].Value.ToString(), out int groupId))
            //{ TellAboutError("Wrong value of curator's grouId"); return; }
            //try { facade = facade.AddCurator((Facade)facade, new Curator(newIdByDefault, name, groupId, email)); }
            //catch (Exception ex) { TellAboutError(ex.Message); return; }
        }
        ...
        private void EditStudentsTable(int rowIndex, int columnIndex, int id)
        {
            switch (columnIndex)
            {
                ...
                case studentGroupIdColumn:
                    ...
                    //if (!int.TryParse(dataGridViewStudents.Rows[rowIndex].Cells[columnIndex].Value.ToString(), out int groupId))
                    //{ TellAboutError("Wrong value of student's groupId"); return; }
                    //try { facade = facade.EditStudentsGroup((Facade)facade, id, groupId); }
                    //catch (Exception ex) { TellAboutError(ex.Message); return; }
                    break;
            }
        }
        ...
        private void EditCuratorsTable(int rowINdex, int columnIndex, int id)
        {
            switch (columnIndex)
            {
                ...
                case curatorGroupNameColumn:
                    ...
                    //if (!int.TryParse(dataGridViewCurators.Rows[rowINdex].Cells[columnIndex].Value.ToString(), out int groupId))
                    //{ TellAboutError("Wrong value of curator's groupId"); return; }
                    //try { facade = facade.EditCuratorsGroup((Facade)facade, id, groupId); }
                    //catch (Exception ex) { TellAboutError(ex.Message); return; }
                    break;
            }
        }
