...
namespace IrisGraphics
{
    public class IrisClass
    {
        ...
        /// </summary>
        public IrisClass(int size)
        {
            values = new MathVector[size];
            for (int i = 0; i < values.Length; i++)
                values[i] = new MathVector((int)Numbers.IRIS_SIZE * size);
        }
        ...
        public void CheckData(string[] file)
        {
            string check;
            try { check = file[0]; }
        ...
        public int DetermineIrisMembership(string irisName, int i)
        {
            int check;
            if (irisName == "setosa")
                check = 0;
                ...
                throw new Exception("Unvalid name of iris");
            return check;
        }

        public IrisClass ResizeIris(IrisClass iris, int amount)
        {
            var newIris = new IrisClass(iris.Length);
            for (int i = 0; i < iris.Length; i++)
            {
                newIris.values[i] = new MathVector(amount);
                for (int j = 0; j < amount; j++)
                    newIris.values[i][j] = iris.values[i][j];
            }
            return newIris;
        }
        ...
        public IrisClass CreateIris(IrisClass iris, string[] array, int irisNumber)
        {
            if (iris.Length == 0)
            ...
            else
            {
                ...
                for (int i = 1; i < array.Length; i++)
                {
                    ...
                    if (currentString.Length < (int)Numbers.METRICS_AMOUNT + 1)
                        throw new Exception("Wrong amount of metrics in string");
                    else if (irisNumber == DetermineIrisMembership(currentString[(int)Numbers.METRICS_AMOUNT], i))
                    {
                        for (int j = 0; j < (int)Numbers.METRICS_AMOUNT; j++)
                        {
                            try { newIris[j][k] = double.Parse(currentString[j], new NumberFormatInfo { NumberDecimalSeparator = "." }); }
                ...
                return ResizeIris(newIris, amount);
            }
        }
        ...
        public IrisClass[] CreateData(string fileName)
        {
            ...
            {
                irisArray[i] = new IrisClass((int)Numbers.METRICS_AMOUNT);
                irisArray[i] = CreateIris(irisArray[i], array, i);
            }
            return irisArray;
        }
